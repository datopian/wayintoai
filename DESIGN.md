...
`index.md` uses Flowershow Markdown mode with standard HTML only; `custom.css` owns layout under the `.wai` scope. The signup form posts to the Substack subscribe page until the list moves (wayintoai-b7z). The same system runs site-wide from `custom.css` on the default Flowershow theme (no `lessflowery`): palette and fonts via Flowershow's theme variables; prose in Newsreader; in-article headings in mono with blue `##`/`###` markers; listings as hairline lists, not cards; a 44rem measure on every page. The file-tree sidebar is off and internal folders are hidden from search (`config.json`). Avoid invented proof points, generic AI imagery and competing calls to action.
